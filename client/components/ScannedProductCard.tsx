...
export const ScannedProductCard: React.FC<ScannedProductCardProps> = ({
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-emerald-500/30 bg-gradient-to-br from-slate-900 via-slate-900 to-emerald-950/20 p-5 backdrop-blur-xl shadow-2xl">
      {/* Top Banner */}
      ...
      <div className="mt-4 flex flex-col gap-4 sm:flex-row sm:items-center">
        {/* Product Image / Icon */}
        <div className="relative flex h-24 w-24 shrink-0 items-center justify-center overflow-hidden rounded-xl border border-slate-800 bg-slate-950 p-2">
          ...
          {product.category && (
            <span className="absolute bottom-1 right-1 rounded bg-slate-900/90 px-1 py-0.5 text-[9px] font-medium text-slate-400">
              {product.category}
            </span>
          )}
        </div>
        ...
        <div className="flex flex-col items-end justify-between gap-3 border-t border-slate-800 pt-3 sm:border-t-0 sm:pt-0">
          <div className="flex items-center gap-2">
          ...
          <div className="text-right">
            ...
            <div className="text-base font-bold text-emerald-400">₹{subtotal}</div>
          </div>
          ...
          <button
            ...
            onClick={handleAdd}
            disabled={isAdding}
            className={`flex items-center gap-2 rounded-xl px-5 py-2.5 text-xs font-semibold shadow-lg transition-all ${
              justAdded
